#!/usr/bin/env python3
"""Bygg EPUB/PDF från projektets kanoniska Markdown-kapitel."""
...
CHAPTER_HEADING_RE = re.compile(r"^#\s+Kapitel\s+(\d+)\s+[–-]\s+(.+?)\s*$", re.MULTILINE)
...
def normalize_chapter_markdown(source: Path, target: Path) -> None:
    ...
    if not match:
        raise RuntimeError(f"{source} saknar kapitelformaterad H1.")
    number, title = match.group(1), match.group(2).strip()
    text = CHAPTER_HEADING_RE.sub(f"# {int(number)}. {title}", text, count=1)

    # Defensivt: om gamla arbetsnoteringar råkat ligga kvar, exportera inte dem.
    note_markers = [
        "\n---\n\nKort kapitelnotering:",
        "\n---\n\n## Kapitelnotering",
        "\n---\n\n## Efter kapitel",
    ]
    cut_positions = [text.find(marker) for marker in note_markers if text.find(marker) != -1]
    if cut_positions:
...
def find_chapters(root: Path) -> list[Path]:
    chapters = sorted((root / "kapitel").glob("kapitel-[0-9][0-9].md"))
    if not chapters:
        raise RuntimeError("Inga kapitelfiler hittades.")
    return chapters
...
def detect_font_dir() -> Path | None:
    ...
    for base in (Path("/usr/share/texmf"), Path("/usr/share/fonts")):
        if not base.exists():
            continue
        matches = list(base.rglob(required["regular"]))
...
def main() -> int:
    ...
    cover_image = metadata["cover-image"]
    ...
    with tempfile.TemporaryDirectory(prefix="roman-build-") as tmp:
        temp = Path(tmp)
        normalized = temp / "chapters"
        normalized.mkdir()
        normalized_chapters: list[Path] = []
        for chapter in chapters:
            out = normalized / chapter.name
            normalize_chapter_markdown(chapter, out)
            normalized_chapters.append(out)
