...
def clean_custom_title_page(epub_dir: Path) -> int:
    """Ta bort Pandocs dubblerade H1 ovanför den egna titelsidan, om den uppstår."""
    changed = 0
    for xhtml in epub_dir.rglob("*.xhtml"):
        tree = ET.parse(xhtml)
        root = tree.getroot()
...
def clean_nav_and_spine(epub_dir: Path, opf_rel: Path, title: str) -> tuple[bool, int]:
    ...
    for nav_item in nav_items:
        ...
        anchors = list(nav_root.findall(".//xhtml:nav[@epub:type='toc']//xhtml:a", NS))
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fix-epub-") as tmp:
        ...
        nav_changed, nav_removed = clean_nav_and_spine(unpacked, opf_rel, title) if title else (False, 0)
